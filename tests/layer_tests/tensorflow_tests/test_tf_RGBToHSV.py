...
class TestRGBToHSV(CommonTFLayerTest):
    def _prepare_input(self, inputs_info):
        ...
        if self.special_case == "Black Image":
            ...
            images_shape = inputs_info['images:0']
            inputs_data = {}
            inputs_data['images:0'] = np.broadcast_to([0.5, 0.5, 0.5], images_shape).astype(self.input_type)
        else:
            images_shape = inputs_info['images:0']
            inputs_data = {} 
    ...
    def create_rgb_to_hsv_net(self, input_shape, input_type, special_case=False):
    ...
    def test_rgb_to_hsv_basic(self, params, ie_device, precision, ir_version, temp_dir,
                                   use_legacy_frontend):
        if ie_device == 'GPU':
            pytest.skip("Accuracy mismatch on GPU")
        self._test(*self.create_rgb_to_hsv_net(**params),
